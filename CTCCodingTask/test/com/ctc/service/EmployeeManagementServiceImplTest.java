...
public class EmployeeManagementServiceImplTest {
	...
	public void testRegisterEmployee() throws ParseException {
		...
		Mockito.when(employeeDao.insertEmployee(Mockito.any(EmployeeBO.class))).thenReturn(1);
		String actualResult = employeeService.registerEmployee(employeeDTO);
		assertEquals("Employee Added Successfully", actualResult);
	}

	@Test
	public void testUpdateEmployeesStatusInActive() {
		Mockito.when(employeeDao.updateEmployeesStatus(new String[] { "EPAM001", "EPAM002" }, "InActive"))
				.thenReturn(2);
		String actualResult = employeeService.updateEmployeesStatus(new String[] { "EPAM001", "EPAM002" }, "InActive");
	...
	@Test
	public void testGetAllEmployeesList() throws ParseException {
		Mockito.when(employeeDao.fetchAllEmployees()).thenReturn(getListOfEmployeesBOTestData());
