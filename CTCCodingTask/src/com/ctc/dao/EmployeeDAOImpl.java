...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	public int insertEmployee(EmployeeBO employeeBO) {
		return jt.update(INSERT_QRY, employeeBO.getId(), employeeBO.getFirstName(), employeeBO.getLastName(),
				employeeBO.getEmail(), employeeBO.getContactNumber(), employeeBO.getDateOfJoining(),
				employeeBO.getStatus());
	}
	...
	private class EmployeeRowMapper implements RowMapper<EmployeeBO> {
		...
		public EmployeeBO mapRow(ResultSet rs, int pos) throws SQLException {
			return new EmployeeBO(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
	...
	public int deleteEmployees(String[] ids) {
		String idsInQueryFormat = listOfIdsInQueryFormat(ids);
		return jt.update(DELETE_EMP_BY_NO.concat(idsInQueryFormat));
	}

	@Override
	public int updateEmployeesStatus(String[] ids, String status) {
		String idsInQueryFormat = listOfIdsInQueryFormat(ids);
		return jt.update(UPDATE_QRY.concat(idsInQueryFormat), status);
	}

	public String listOfIdsInQueryFormat(String[] ids) {
